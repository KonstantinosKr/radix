...
int main (){
	...
	for(unsigned int i=0;i<kLength+1;i++){
		for(unsigned int y=0;y<5;y++){
            if(!(digit = number[y])/10==0)
                for(unsigned int k=0;k<i;k++) digit = digit/10; // got to digit index
			if((digit = digit%10)==0) digit=1; //if digit = 0 the set to 1 to keep in array boundary
